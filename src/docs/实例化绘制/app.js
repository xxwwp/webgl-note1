...
void main () {
  ...
  float row = floor(a_Offset / a_Sum);
  float col = mod(a_Offset , a_Sum);
  position.x += (col - a_Sum / 2.0)  * 3.0;
  position.y += (row - a_Sum / 2.0) * 3.0;

  gl_Position = u_Matrix *  position ;
  v_Position = u_MatrixModel * position; // 对顶点进行插值
  v_Color = a_Color;
...
function main() {
  ...
  if (extInstanced === null) throw Error("不支持 ANGLE_instanced_arrays 扩展");
  ...
  function render() {
    ...
    drawCube({ preMatrix, extInstanced });
  }
  const sum = 100;
  const offsets = new Float32Array(Array.from({ length: sum ** 2 }).map((_, i) => i));
  /**
   *
   * @param {{
   * preMatrix: Float32Array; // 预设矩阵，本次渲染除去模型矩阵以外的的矩阵
   * extInstanced: ANGLE_instanced_arrays; // ANGLE_instanced_arrays 扩展
   * }} param0
   */
  function drawCube({ preMatrix, extInstanced }) {
    // 模型矩阵
    let mat4Model = mat4.fromScaling(mat4.create(), new Float32Array([0.5, 0.5, 0.5]));
    mat4Model = mat4.rotateZ(mat4.create(), mat4Model, modelParam.zScale);
    ...
    glSet32ArrayBuffer({
      gl,
      index: getLocaltiona(gl, "a_Offset"),
      value: offsets,
      size: 1,
      stride: 1,
      offset: 0,
    });
    gl.uniform1f(getLocaltionu(gl, "a_Sum"), sum); // 指定每行有多少正方体
    extInstanced.vertexAttribDivisorANGLE(getLocaltiona(gl, "a_Offset"), 1); // 指定实例化数据
    extInstanced.drawElementsInstancedANGLE(gl.TRIANGLES, n, gl.UNSIGNED_BYTE, 0, offsets.length);
  }

  function frameLoop() {
    requestAnimationFrame(() => {
      modelParam.zScale += 0.0005;
      render();
      frameLoop();
    });
  }
